...
async function main() {
  const { BASE_PATH, CURRENT_RPC_URL, TRUSTED_FORWARDER } = process.env;

  const Box = await ethers.getContractFactory('L402');
    const box = await upgrades.deployProxy(Box, [42, TRUSTED_FORWARDER]); // where "42" is the argument, i.e. initialize(42)
    await box.deployed();
